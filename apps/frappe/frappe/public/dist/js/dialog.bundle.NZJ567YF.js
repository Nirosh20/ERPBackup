(() => {
    ...
    Link: function(value, docfield, options, doc) {
      ...
      if (docfield && docfield.link_onclick) {
        ...
        if (frappe.model.can_read(doctype)) {
          ...
          a.href = `/app/${encodeURIComponent(frappe.router.slug(doctype))}/${encodeURIComponent(original_value)}`;
  ...
  var Tab = class {
    ...
    refresh() {
      ...
      if (!hide) {
        hide = true;
        if (this.wrapper.find(".form-section:not(.hide-control, .empty-section), .form-dashboard-section:not(.hide-control, .empty-section)").length) {
          hide = false;
        }
      }
      this.toggle(!hide);
            ...
            let updated = (this.target.frm.doc[this.target.df.fieldname] || []).some((d) => {
              if (d[this.fieldname] === value) {
                frappe.model.set_value(d.doctype, d.name, this.qty_fieldname, data.qty).then(() => {
                  frappe.show_alert(__("Added {0} ({1})", [
                    value,
...
//# sourceMappingURL=dialog.bundle.NZJ567YF.js.map
